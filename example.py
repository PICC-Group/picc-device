...
def setup():
    ...
    ref_angle_data = [
        get_angle(*data_to_np(mindist_minangle)),
        get_angle(*data_to_np(mindist_maxangle)),
        get_angle(*data_to_np(maxdist_minangle)),
        get_angle(*data_to_np(maxdist_maxangle)),
    ]

    # [average throttle at min distance, average throttle at max distance]
    ref_throttle_data = [
        (
            get_throttle(*data_to_np(mindist_minangle))
            + get_throttle(*data_to_np(mindist_minangle))
        )
        / 2,
        (
            get_throttle(*data_to_np(maxdist_minangle))
            + get_throttle(*data_to_np(maxdist_maxangle))
        )
        / 2,
    ]
